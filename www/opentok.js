...
// Helper methods
//
var getPosition = function getPosition(divName) {
...
var tbSuccess = function tbSuccess() {
  // console.log('success');
};

var tbGetZIndex = function tbGetZIndex(element) {
  var ele = element;
  while (ele) {
...
var tbGetScreenRatios = function tbGetScreenRatios() {
  return {
...
var tbGetBorderRadius = function tbGetBorderRadius(element) {
  var ele = element;
  while (ele) {
...
var pdebug = function pdebug(msg, data) {
...
var TBPublisher = function () {
  function TBPublisher(targetElement, properties, completionHandler) {
    ...
    Cordova.exec(this.eventReceived.bind(this), tbSuccess, OTPlugin, 'addEvent', ['publisherEvents']);
  }
...
!function (window, undefined) {
  ...
  var OTHelpers = function OTHelpers(domId) {
  ...
  OTHelpers.isArray = OTHelpers.isFunction(Array.isArray) && Array.isArray || function (vArg) {
  ...
  var _browser = function () {
    ...
    } else if ((navigatorVendor = window.navigator.vendor) && navigatorVendor.toLowerCase().indexOf('apple') > -1) {
      browser = 'Safari';
...
(function (window, OTHelpers, undefined) {
  ...
  OTHelpers.useLogHelpers = function (on) {
    ...
    var makeLogArgumentsSafe = function makeLogArgumentsSafe(args) {
      return args;
    };

    if (OTHelpers.browser() === 'IE') {
      makeLogArgumentsSafe = function makeLogArgumentsSafe(args) {
...
(function (window, OTHelpers, undefined) {
  ...
  */
  OTHelpers.eventing = function (self, syncronous) {
    ...
    var removeAllListenersNamed = function removeAllListenersNamed(eventName, context) {
    ...
    var removeListeners = function removeListeners(eventNames, handler, context) {
    ...
    */
    ...
    if (env) {
      addListeners = jasmine.createSpy('spy on OTHelpers.eventing.addListeners').andCallFake(addListeners);
      removeListeners = jasmine.createSpy('spy on OTHelpers.eventing.removeListeners').andCallFake(removeListeners);
      removeAllListenersNamed = jasmine.createSpy('spy on OTHelpers.eventing.removeAllListenersNamed').andCallFake(removeAllListenersNamed);
...
(function (window, OTHelpers, undefined) {
  ...
  OTHelpers.isElementNode = function (node) {
    return node && (typeof node === 'undefined' ? 'undefined' : babelHelpers.typeof(node)) === 'object' && node.nodeType == 1;
  };

  // Returns true if the client supports element.classList
  OTHelpers.supportsClassList = function () {
    var hasSupport = babelHelpers.typeof(document !== "undefined") && "classList" in document.createElement("a");
    OTHelpers.supportsClassList = function () {
      return hasSupport;
    };
  ...
  OTHelpers.createElement = function (nodeName, attributes, children, doc) {
    ...
    if (attributes) {
      for (var name in attributes) {
        if (babelHelpers.typeof(attributes[name]) === 'object') {
...
(function (window, OTHelpers, undefined) {
  ...
  OTHelpers.getComputedStyle = function (element) {
    if (element && element.ownerDocument && element.ownerDocument.defaultView && element.ownerDocument.defaultView.getComputedStyle) {
...
(function (window, OTHelpers, undefined) {
  ...
  var _width = function _width(element) {
    ...
    return OTHelpers.css(element, 'width');
      ...
      _height = function _height(element) {
